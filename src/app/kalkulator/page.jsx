...
import { Calculator, ArrowLeft, Save, Info, RotateCcw, CheckCircle2, Calendar as CalendarIcon, Clock, Sparkles } from 'lucide-react';
...
export default function KalkulatorPage() {
  ...
  const handleSaveAndReturn = (e) => {
    ...
    try {
      ...
      // Dispatch custom event to notify home dashboard
  ...
  const handleReset = () => {
    if (confirm('Reset tanggal ke 1 tahun terakhir?')) {
        ...
        <div className="bg-emerald-950 text-white rounded-3xl p-6 md:p-8 shadow-xl border border-emerald-800">
          <div className="flex items-center gap-3.5 mb-2">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-300">Pilih rentang tanggal tidak sholat (Calendar Picker) untuk menghitung total hutang sholat fardhu</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
              <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>Pilih Rentang Tanggal (Calendar Picker)</span>
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">Tentukan tanggal mulai dan tanggal akhir sholat yang pernah ditinggalkan</p>
            </div>
            <button
              onClick={handleReset}
            ...
            <div className="p-5 rounded-2xl bg-emerald-50 border border-emerald-200/90 space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-emerald-200/80 pb-3">
                <div className="flex items-center gap-2 text-emerald-950">
                  ...
                  <span className="text-sm font-bold">Ringkasan Rentang Tanggal:</span>
                </div>
                <div className="text-xs font-semibold text-emerald-900 bg-white px-3 py-1 rounded-full border border-emerald-200">
                  {formatReadableDate(startDate)} &mdash; {formatReadableDate(endDate)}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="bg-white p-3.5 rounded-xl border border-emerald-100 shadow-sm">
                  ...
                  <div className="text-[11px] text-slate-400 font-medium mt-0.5">Subuh, Dzuhur, Ashar, dll</div>
                </div>
                ...
                <div className="bg-white p-3.5 rounded-xl border border-emerald-100 shadow-sm">
                  ...
                  <div className="text-[11px] text-slate-400 font-medium mt-0.5">Total akumulasi 5 waktu</div>
                </div>
              </div>
            </div>
